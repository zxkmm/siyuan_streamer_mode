...
export default class siyuan_streamer_mode extends Plugin {
  ...
  blackOutKeyWords(_keywords_array_) {
    ...
    const keywords = _keywords_array_;
      ...
      keywords.forEach((keyword) => {
        if (text.includes(keyword)) {
          highlightedText = highlightedText.replace(
            ...
            `<span style="color: black; background-color: black;">${keyword}</span>`
          );
        }
      });
  ...
  init_event_bus_handler(){
    if(this.settingUtils.get("totalSwitch")){
      ...
      const _blacklist_words_ = this.convertStringToArray(this.settingUtils.get("keywordsBlacklist"));

      if(this.settingUtils.get("eventBusSwitchProtyleSwitch")){
        this.eventBus.on("switch-protyle", () => this.blackOutKeyWords(_blacklist_words_));
      }

      if(this.settingUtils.get("eventBusClickEditorcontentSwitch")){
        this.eventBus.on("click-editorcontent", () => this.blackOutKeyWords(_blacklist_words_));
      }

      if(this.settingUtils.get("eventBusWsMainSwitch")){
        this.eventBus.on("ws-main", () => this.blackOutKeyWords(_blacklist_words_));
      }
    }
  }
